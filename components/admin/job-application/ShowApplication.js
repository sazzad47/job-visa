  ...
  <>
    ...
    >
      <SimpleShowLayout id="exportToPDF">
        ...
        {jobInfo.map((item, i) =>
          item.type === "text" ? (
            <TextField key={i} label={item.label} source={item.value} />
          ) : item.type === "photo" ? (
            <ImageField key={i} label={item.label} source={item.value} />
          ) : (
            <FileField key={i} title={item.label} source={item.value} />
          )
        )}
