...
const JobChecking = () => {
  ...
  const handleCheck = async (e) => {
    e.preventDefault()
    setLoading(true)
    const res = await getData(`jobChecking?index=${jobAppID}&passportNumber=${passportNum}`)
    ...
    return (
        ...
        >
          {!data && <Box style={{backgroundColor: '#4331ad', color: '#d4d1e8', marginBottom: '1rem' }}><Typography style={{padding: '1rem', fontSize: '1rem'}}>Invalid passport number or Job Application ID!</Typography></Box>}
          ...
          <Box component="form" onSubmit={handleCheck} sx={{ mt: 1 }}>
            ...
            (data?<Box sx={{width:'100%'}}>
              <Typography sx={{fontWeight: 'bold', margin: '2rem 0'}} align="center">Job Application No {data?.index}</Typography>
              <Grid container>
