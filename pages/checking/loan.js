...
const LoanChecking = () => {
  ...
  const handleCheck = async (e) => {
    e.preventDefault()
    setLoading(true)
    const res = await getData(`loanChecking?index=${laonAppID}&idNumber=${idNumber}`)
        ...
        >
          ...
          <Box component="form" onSubmit={handleCheck} sx={{ mt: 1 }}>
            ...
            (data?<Box sx={{width:'100%'}}>
              <Typography sx={{fontWeight: 'bold', margin: '2rem 0'}} align="center">Loan Application No {data?.index}</Typography>
              <Grid container>
                <Grid item xs={12} md={6}>ID Number:</Grid>
                <Grid item xs={12} md={6}>{data?.idNumber}</Grid>
                <Grid item xs={12} md={6}>Laon Amount:</Grid>
                <Grid item xs={12} md={6}>{data?.loanAmount}</Grid>
                <Grid item xs={12} md={6}>Amount of Money:</Grid>
                <Grid item xs={12} md={6}>{data?.amountOfMoney}</Grid>
