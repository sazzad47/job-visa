...
const updateStatus = async (req, res) => {
  try {
    ...
    const application = await JobApplicants.findOneAndUpdate(
      { _id: id },
      { status }
...
const updateCost = async (req, res) => {
  try {
    ...
    const application = await JobApplicants.findOne({ _id: id }).populate(
      "user",
      "-password"
    );
    if (cost) {
      await JobApplicants.findOneAndUpdate(
        { _id: id },
        { cost: parseInt(cost) }
      );
